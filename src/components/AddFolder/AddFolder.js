...
const AddFolder = ({all,setAll,folder,setFolder,addFolder,setAddFolder}) => {
    ...
    const addFolderInAll = (e) => {
        e.preventDefault()
        if (all.map((item)=> item.name).includes(folder)){
    ...
    return (
        <div className="sidebar__add">
            <button type="button" className="sidebar__add-btn" onClick={()=> setAddFolder(!addFolder)}><span className="sidebar__add-btn-span">+</span>Добавить задачу</button>
            <div style={{display : `${addFolder ? 'block' : 'none'}`}} className="sidebar__add-form">
                <form action="" onSubmit={addFolderInAll}>
                    ...
                    <ul className="sidebar__add-form-colors">
                        {colors.map((item,idx)=>(
                            <li
                                ...
                                key={item}
                                className={`sidebar__add-form-circle ${active === idx ? 'active' : ''}`}
                ...
                <button onClick={()=> {
                    setAddFolder(false)
                    setFolder('')
                }} className="sidebar__add-form-close"><svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
